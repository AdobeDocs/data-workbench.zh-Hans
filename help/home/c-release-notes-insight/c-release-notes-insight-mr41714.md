---
description: Adobe Analytics将更新并增强其数据馈送功能，为Data Workbench注入强大动力，以利用Adobe Experience Cloud中的档案和受众。 所有 Data Workbench 用户需要在 2014 年 4 月 21 日之前准备好过渡到该新数据源格式。
title: 2014 年 4 月修改发布更新
...
# 2014 年 4 月修改发布更新{#updates-for-april-mr}

Adobe Analytics将更新并增强其数据馈送功能，为Data Workbench注入强大动力，以利用Adobe Experience Cloud中的档案和受众。 所有 Data Workbench 用户需要在 2014 年 4 月 21 日之前准备好过渡到该新数据源格式。

档案和受众可提供完整、完全可操作的客户视图，以便在Adobe Experience Cloud和第三方服务中使用。 为了开始为Analytics中的新功能奠定基础，新的Experience Cloud ID服务将在即将发布的4月MR版本中添加到数据源，并提供其他增强和改进。 使用Adobe Experience Cloud的任何客户均可使用此新服务。

在该主题下面列出了随该新数据源提供的额外增强功能和错误修复，以及准备 Data Workbench 配置的说明。

## 新数据源的增强功能和问题修复 {#section-0013de07da93411d929a1dfd0417f61b}

* 添加了Experience Cloud ID服务。
* 添加“目标操作”字段以支持 Analytics 中的目标报告和分析。
* 在产品行类型中添加 [!DNL bot_id] 值以便与 Report &amp; Analytics 保持一致。
* 降低促销 eVar 大小而不破坏数据值的完整性。
* 解决了以下问题：有时在促销 eVar 中显示哈希值。
* 在流程中进行了内部改进以继续提供更好的稳定性和性能。

## 更新 Data Workbench 配置 {#section-381fcd185af6474bb656501c096c451d}

可通过更改 Data Workbench 以确保在过渡后不间断地集成数据源。在下一计划重新处理期间，将更新配置的日志处理阶段中的当前解码器组。负责管理Data Workbench体系结构的团队将能够执行其他任务，以帮助展示和利用新的Experience Cloud访客标识符。

**用于更新 Data Workbench 的任务**

1. 查找当前的解码器组。通常情况下，位于配置文件管理器的 Dataset\Log Processing\Decoding [!DNL Instructions.cfg] 中。
1. 在该配置文件中，右键单击当前的解码器格式，然后选择“复制”。
1. 右键单击，然后在当前解码器格式下面粘贴新条目。
1. 展开新的解码器格式，并在列表末尾添加两个新字段位置。如果未准备好在数据集中使用新字段，则可以将这些位置保留空白，或者按如下方式定义这些位置。

   * [!DNL x-mcvisid]
   * [!DNL x-tnt-action]

## 解码说明 {#section-ba63132ce9604851ad319b6181011820}
