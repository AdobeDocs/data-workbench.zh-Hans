...
# 定义查找转换{#defining-lookup-transformations}
...
A [!DNL Categorize] 与使用 [!DNL Regular Expression] 转变来实现同样的目标。

>[!NOTE]
>
>中使用的子字符串测试 [!DNL Categorize] 区分大小写，除非使用 [!DNL Case Sensitive] 参数。
...
* 对 [!DNL Categorize] 在 [!DNL Transformation.cfg] 文件或 [!DNL Transformation Dataset Include] 文件需要重新转换数据集。 的查找文件 [!DNL Categorize] 在 [!DNL Log Processing.cfg] 文件或 [!DNL Log Processing Dataset Include] 文件不受此限制的约束。 有关重新处理数据的信息，请参阅 [重新处理和重新转换](../../../../home/c-dataset-const-proc/c-reproc-retrans/c-unst-reproc-retrans.md).

* [!DNL Categorize] 在 [!DNL Log Processing.cfg] 文件或 [!DNL Log Processing Dataset Include] 文件会在查找文件发生更改时重新加载其查找文件。 更改不可向前应用，但它们会应用于更改发生之后读取的所有日志数据。
...
输出基于子字符串在对照文件中的顺序。例如，cs-uri-stem [!DNL /sports/products/buy.php] 返回“产品”。 尽管 URI 主干以“/sports/”开头，但在对照文件中，字符串“/products/”列在“/sports/”的前面。如果 Multiple Values（多个值）参数设为 true，则 x-custommap 将再具有一个值，例如最后一个示例将匹配对照表中的两行：“产品”和“新闻”。
...
* 对 [!DNL FlatFileLookup] 在 [!DNL Transformation.cfg] 文件或 [!DNL Transformation Dataset Include] 文件需要重新转换数据集。 的查找文件 [!DNL FlatFileLookup] 在 [!DNL Log Processing.cfg] 文件或 [!DNL Log Processing Dataset Include] 文件不受此限制的约束。 有关重新处理数据的信息，请参阅 [重新处理和重新转换](../../../../home/c-dataset-const-proc/c-reproc-retrans/c-unst-reproc-retrans.md).

* [!DNL FlatFileLookup] 中的转换 [!DNL Log Processing.cfg] 文件或 [!DNL Log Processing Dataset Include] 文件会在查找文件发生更改时重新加载其查找文件。 更改不可向前应用，但它们会应用于更改发生之后读取的所有日志数据。
...
该示例转换在cs(referrer-query)字段中搜索PartnerID名称 — 值对，如果找到，则搜索查找文件 [!DNL Lookups\partners.txt] 用于将PartnerID值与 [!DNL Partner] 列。 如果找到行，则会为输出字段x-partner-name指定 [!DNL PrintName] 列。
...
的 [!DNL ODBCLookup] 转换操作类似于 [!DNL FlatFileLookup] 转换。 唯一的区别是此转换期间使用的对照表是从 ODBC 数据库而不是无格式文件填充的。
