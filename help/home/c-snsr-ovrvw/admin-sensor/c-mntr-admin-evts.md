---
description: 要尽快检测传感器错误并修复它们，以免造成严重问题或中断，您应定期监控事件日志。
title: 监控管理事件
...
要尽快检测传感器错误并修复它们，以免造成严重问题或中断，您应定期监控事件日志。

**推荐频率：** 至少每小时

您可以使用Windows事件查看器或Unix Syslog文件以及默认位于[!DNL Sensor]安装目录[!DNL Logs]文件夹中的[!DNL *.sensor-log]文件来监视这些事件。 这些文件表示在数据收集过程中存在错误，尤其是当[!DNL Sensor]无法连接到目标[!DNL data workbench server]和开始队列数据时。
...
syslog守护程序根据您在syslog.conf文件中指定的规则将错误消息写入日志文件。 根据严重性的不同，错误记录在标志“LOG_DAEMON”和“LOG_NOTICE”或“LOG_ERR”中。
...
所有传感器消息都包含字符串&quot;Sensor&quot;，并进行编号以反映所显示消息的重要性。
...
>[!NOTE]
>
>警告(2xxx)当前未使用。 这些数字保留供将来使用。

您的网络管理工具可设置为每5-10分钟监视一次消息，以发现“传感器”源出错，并提醒相应人员可能需要干预的问题。 您可以选择只监视某些类型的事件消息（如“传感器错误”字符串）的系统。 或者，也可以对预设“传感器信息”、“传感器警告”和“传感器错误”字符串的事件应用不同的规则。

## 识别重要消息{#section-5a20f5dc18ca4012931d194db855e54e}

在事件日志中，您应特别注意并立即处理与队列大小相关的任何消息。

例如，“ [!DNL Sensor Info 1012: Adobe disk queue is #% full]”等消息需要注意。
...
<table id="table_F8835AC0AD8F43E2B4494D8D35EBC0FD"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> 每次磁盘队列利用率超过10%阈值时都会记录此消息。 如果此百分比继续增加，则应在队列已满并丢失数据之前采取操作。 最可能的问题是传感器已停止与Insight Server通信。 联系Adobe ClientCare。 </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td colname="col1"> 传感器信息1017:加载了实验文件文件名 </td> 
   <td colname="col2"> 无需执行任何操作。 传感器已成功加载列出的实验文件。 </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 传感器错误3016:无法加载配置文件/mypath/myfile </td> 
   <td colname="col2"> 确认在Web服务器配置中指定的传感器配置文件是否存在，并具有Web服务器进程读取所需的权限。 </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> 项目可能由外部信号终止。 如果无法确定此信号的源，请与Adobe ClientCare联系。 </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 传感器错误5035:从主区外调用退出 </td> 
   <td colname="col2"> 联系Adobe ClientCare。 </td> 
  </tr> 
  <tr> 
...
| 传感器错误3019:vys-cookie未在vys-log之前调用。 请联系支持人员。 | 联系Adobe ClientCare。 |
| 传感器错误3025:子请求点返回给自己 | 联系Adobe ClientCare。 |

**AOL Server**

| 事件消息 | 建议的操作 |
|---|---|
| 传感器错误3015:ns/server/[server]/module/[module]部分在AOLServer配置文件中缺失。 | 这是配置错误。 正确无误。 |
| 传感器错误3019:vys-cookie未在vys-log之前调用。 请联系支持人员。 联系Adobe ClientCare。 | 请联系支持人员。 联系Adobe ClientCare。 |
| 传感器错误3020:VisualSciencesConfig缺失为AOLServer配置文件中[section]部分中的第一个条目。 | 这是配置错误。 正确无误。 |
...
| 事件消息 | 建议的操作 |
|---|---|
| 传感器错误3011:需要Init指令。 如Init fn=vys-init config-file=&quot;/mypath/myfile&quot; | 这是配置错误。 缺少iPlanet init指令。 |
| 传感器错误3015:在iPlanet Init指令中未指定config-file | 这是配置错误。 iPlanet Init指令中未提供配置文件的路径。 |
| 传感器错误3019:vys-cookie未在vys-log之前调用。 请检查配置文件 | 必须将vys-cookie指定为每个软件虚拟服务器的first NameTrans指令。 |
